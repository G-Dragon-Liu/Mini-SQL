...
struct TupleData {
	...
	bool operator < (const TupleData& cmp) {
		if (flag == -2) {
			int a, b;
			a = (unsigned char)tuple_data[0];			a << 8;
			a += (unsigned char)tuple_data[1];			a << 8;
			a += (unsigned char)tuple_data[2];			a << 8;
			a += (unsigned char)tuple_data[3];
			b = (unsigned char)cmp.tuple_data[0];			b << 8;
			b += (unsigned char)cmp.tuple_data[1];			b << 8;
			b += (unsigned char)cmp.tuple_data[2];			b << 8;
	...
	bool operator == (const TupleData& cmp) {
		if (flag == -2) {
			int a, b;
			a = (int)tuple_data[0]; a *= 256;
			a += (int)tuple_data[1]; a *= 256;
			a += (int)tuple_data[2]; a *= 256;
			a += (int)tuple_data[3];
			b = (int)cmp.tuple_data[0]; b *= 256;
			b += (int)cmp.tuple_data[1]; b *= 256;
			b += (int)cmp.tuple_data[2]; b *= 256;
			b += (int)cmp.tuple_data[3];
			return a == b;
		}
		else if (flag == -1) {
			float *a, *b;
...
public:
	int isempty = 1;
	BTree() {}							// default ctor
	BTree(string table, string attribute, string indexname, int flag, int k = 15) :table(table), attribute(attribute), indexname(indexname), k(k), flag(flag) {
		Head = nullptr;
	}
	~BTree() {
